...
function ecdhDeriveFn() {
  ...
  var fallback = function(key, props) {
    ...
    var p = Promise.all([
      ecUtil.convertToElliptic(key, false),
      ecUtil.convertToElliptic(props.public, true),
      ecUtil.curveSize(key.crv, true)
    ]);
    p = p.then(function(args) {
      // {privKey} is elliptic.KeyPair
      var privKey = args[0];
      // {pubKey} is elliptic.Point
      var pubKey = args[1].getPublic();
      var pad = args[2];

      var secret = privKey.derive(pubKey).toBuffer(null, pad);
      if (keyLen) {
  ...
  var webcrypto = function(key, props) {
    ...
    // assume {props.public} is publicKey
    var pubKey = Promise.resolve(ecUtil.convertToJWK(props.public, true));
  ...
  var nodejs = function(key, props) {
    ...
    var p = Promise.all([
      ecUtil.convertToBuffer(key, false),
      ecUtil.convertToBuffer(props.public, true)
    ]);
    p = p.then(function(all) {
      // {pubKey} is "buffer"
      var privKey = all[0],
          pubKey = all[1];
