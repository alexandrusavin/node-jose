...
function ecdsaSignFN(hash) {
  ...
  var fallback = function(key, pdata /*, props */) {
    ...
    var pk = ecUtil.convertToElliptic(key, false);
    ...
    promise = promise.then(function(result) {
      var pad = ecUtil.curveSize(key.crv, true);
      result = pk.sign(result);
      result = Buffer.concat([
        result.r.toBuffer(null, pad),
        result.s.toBuffer(null, pad)
      ]);
      return {
        data: pdata,
        mac: result
...
function ecdsaVerifyFN(hash) {
  ...
  var fallback = function(key, pdata, mac /*, props */) {
    ...
    var pk = ecUtil.convertToElliptic(key, true);
    ...
    promise = promise.then(function(result) {
      ...
      var rs = {
        r: mac.slice(0, len).toString("hex"),
        s: mac.slice(len).toString("hex")
      };
