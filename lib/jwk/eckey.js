...
var JWKEcFactory = {
  ...
  generate: function(size) {
    var ec = CURVES[size];
    if (!ec) {
      return Promise.reject(new Error("unsupported curve: " + size));
    }

    function genPrivate() {
      var curve = ec.curve;
      var d = forge.random.getBytes(ecutil.curveSize(size, true)),
          n = new forge.jsbn.BigInteger(curve.n.toString(16), 16);
      d = new Buffer(d, "binary").toString("hex");
      d = new forge.jsbn.BigInteger(d, 16);
      d = d.mod(n.subtract(forge.jsbn.BigInteger.ONE)).
            add(forge.jsbn.BigInteger.ONE);
      return d.toString(16);
    }

    var keypair = ec.keyFromPrivate(genPrivate());
    var privkey = keypair.getPrivate(),
        pubkey = keypair.getPublic(),
        len = ecutil.curveSize(size, true);
    var result = {
      "crv": size,
      "x": pubkey.getX().toBuffer(null, len),
      "y": pubkey.getY().toBuffer(null, len),
      "d": privkey.toBuffer(null, len)
    };
